...
def consultaSeExisteUsuario(acao, usuario):
    try:
        ...
        logging.warning(f"Falha na automação consultaSeExisteUsuario: {e}")
...
def consultaIdUsuarioByUserDigito(usuario):
    try:
        ...
        logging.warning("Falha na automação consultaIdUsuarioByUserDigito : " + e)
...
def consultaIdAcaoByNomeAcao(acao):
    try:
        ...
        if not linha:
            conn.close()
            logging.info(f'Id não existe, será cadastrado a ação {acao}')
...
def insertTotbBaixaUsuariosPorAcoes(acao_id, usuario_id):
    ...
    try:
        ...
        logging.warning(f'Falha na automação insertTotbBaixaUsuariosPorAcoes, {acao_id} {usuario_id} \n {e}')
...
def deletaUsuarioNaAcao(usuario, acao):
    try:
        ...
        logging.warning(f'Falha na automacão deletaUsuarioNaAcao: {e}')
...
def cadastraNovaAcao(acao):
    try:
        ...
        conn = conectaBanco(bancoAtual)
        cursor = conn.cursor()
        tem_acao = consultaAcoes()
        if acao not in tem_acao:
            ...
            logging.warning('acao já existe no banco')
    except Exception as e:
